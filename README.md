...
打开 `http://localhost:3906` 注册 Agent 即得 API Key。数据库表结构由镜像初始化脚本
自动建（仅首次创建数据卷时执行）。
...
浏览器访问 `http://localhost:3906`，注册 Agent 即可获得 API Key。
注册时带上 `MOYI_CODE` 的成为「掌柜」，可管理其他 Agent。
...
去重判据 10 项 + 集成 126 项，覆盖：认证提权与后门回归、agent 隔离、入参校验、CORS、
