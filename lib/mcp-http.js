...
const TOOLS = [
  ...
  {
    name: 'skill_propose',
    description: '把你摸索出的一份操作手册/流程提交成【技能草稿】。草稿只有你自己看得到，需管理员在管理台点"发布"后才进入所有 Agent 共享的公共技能层。正文用 Markdown；建议在开头写 YAML frontmatter（name + description），否则由正文首个标题或 source_url 推断。',
    aliases: [],
    inputSchema: {
      type: 'object',
      properties: {
        ...
        description: { type: 'string', description: '一句话用途描述，用于清单展示。不传则由正文推断。' },
        source_url: { type: 'string', description: '可选：该技能的原始出处链接，仅作溯源，不会替你抓取。' },
      },
      required: ['content'],
    },
    rest: (a) => ({ method: 'POST', path: '/api/skills',
      body: { content: a.content, name: a.name, description: a.description, url: a.source_url } }),
  },
  {
    name: 'skill_list',
    ...
    aliases: [],
    inputSchema: { type: 'object', properties: {} },
    rest: () => ({ method: 'GET', path: '/api/skills' }),
  },
  {
    name: 'skill_read',
    ...
    rest: (a) => {
      ...
      if (!/^[a-z0-9][a-z0-9._-]{0,63}$/.test(n)) return { error: '技能名格式不合法' };
      return { method: 'GET', path: '/api/skills/' + encodeURIComponent(n) };
    },
  },
];
...
function renderResult(tool, args, r) {
  ...
  switch (tool.name) {
    ...
    case 'skill_propose': {
      const s = r.skill || {};
      return '已提交为技能草稿（仅你可见）。\n  名称: ' + (s.name || '-')
        + '\n  说明: ' + (s.description || '-') + '\n  状态: ' + (r.status || 'draft')
        + '\n\n' + (r.note || '需管理员在管理台发布后才会对所有 Agent 可见。');
    }
    case 'skill_list': {
      const pub = r.skills || [], dr = r.drafts || [];
      const lines = [];
...
/**
 * resources/list —— 把技能层映射成 MCP 资源（uri = moyi-skill://<name>）。
 *
 * 实现上仍然走一次内部 REST（callRest），与 tools 同一原则：不在这里复制
 * 可见性判断。技能表还没建时回空清单而不是报错——资源列表是能力探测阶段就
 * 会被调用的东西，一个尚未启用技能功能的实例不该因此连不上。
 */
async function listResources(ctx) {
  let r;
  try { r = await ctx.callRest('GET', '/api/skills'); }
  catch { return []; }
  if (!r || r._status >= 400 || r.error) return [];
  const out = [];
  for (const s of (r.skills || [])) {
    out.push({ uri: s.uri || SK.resourceUri(s.name), name: s.name,
      title: s.name, description: s.description || '', mimeType: 'text/markdown' });
  }
  for (const s of (r.drafts || [])) {
    out.push({ uri: s.uri || SK.resourceUri(s.name), name: s.name,
...
/** resources/read —— 按 uri 取全文。找不到一律 -32002，与 MCP 规范一致。 */
async function readResource(uri, ctx) {
  const name = SK.nameFromUri(uri);
  // 不校验就拼路径，等于把任意字符串塞进内部路由；先按技能名的规矩过一遍。
  if (!name || !/^[a-z0-9][a-z0-9._-]{0,63}$/.test(name)) {
    return { error: { code: -32002, message: 'Unknown resource: ' + String(uri) } };
  }
  let r;
  try { r = await ctx.callRest('GET', '/api/skills/' + encodeURIComponent(name)); }
  catch (e) { return { error: { code: -32603, message: '读取技能失败: ' + e.message } }; }
  if (!r || !r.skill || typeof r.content !== 'string') {
    return { error: { code: -32002, message: 'Unknown resource: ' + String(uri) } };
  }
  return { result: { contents: [{ uri: r.skill.uri || SK.resourceUri(name),
    mimeType: 'text/markdown', text: r.content }] } };
}
...
async function handleRpc(msg, ctx) {
  ...
  switch (msg.method) {
    ...
    case 'resources/list':
      return { jsonrpc: '2.0', id, result: { resources: await listResources(ctx) } };
    case 'resources/read': {
      const out = await readResource((msg.params || {}).uri, ctx);
      return out.error ? { jsonrpc: '2.0', id, error: out.error }
        : { jsonrpc: '2.0', id, result: out.result };
    }
    case 'prompts/list':
