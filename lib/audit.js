 ...
 *
 * 下面三个原语是通用的（Agent key 与管理台口令各用一套独立的键空间），
 * 因为两者如果被同一个计数器管着，一个人忘了 Agent 口令就能把管理员
 * 一起锁在门外，反之亦然。
 */
function failGate(mapKey, limit, windowMs) {
  const bs = windows.get(mapKey) || [];
  const cutoff = Date.now() - windowMs;
  const used = bs.filter(b => b.t + 60000 >= cutoff).reduce((s, b) => s + b.n, 0);
  if (used >= limit) return { allowed: false, reason: 'fail_limit', limit, retry_after_ms: windowMs };
  return { allowed: true };
}
function failAdd(mapKey, windowMs) {
  windowAdd(mapKey, Number.MAX_SAFE_INTEGER, windowMs);
}
function failClear(mapKey) {
  windows.delete(mapKey);
}

function authGate(req) {
  const limit = Math.max(1, parseInt(process.env.MOYI_AUTH_FAIL_PER_15MIN, 10) || 30);
  return failGate('auth:' + clientKey(req), limit, 900000);
}
function authFailed(req) {
  failAdd('auth:' + clientKey(req), 900000);
}
function clearAuthFail(req) {
  failClear('auth:' + clientKey(req));
}
...
module.exports = {
  ...
  failGate, failAdd, failClear, windowAdd,
  _reset,
};
