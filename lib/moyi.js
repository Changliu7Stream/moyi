...
const { vitality, decayFactor, evaluate: evaluateDecay, HALFLIFE_DAYS, FORGET_AFTER } = require('./decay.js');
...
const DEDUPE_VEC = numEnv('MOYI_DEDUPE_VEC', 0.90);
const DEDUPE_VEC_LOCAL = numEnv('MOYI_DEDUPE_VEC_LOCAL', 0.70);
const DEDUPE_LEX = numEnv('MOYI_DEDUPE_LEX', 0.88);
const DEDUPE_LEX_PROVIDER = numEnv('MOYI_DEDUPE_LEX_PROVIDER', 0.60);
...
function dbError(res, r) {
  ...
  return json(res, 502, {
    ...
    hint: '常见原因：' + ENV_KEY + ' 无权限（Supabase 开启 RLS 后 anon key 会被拒）、'
      + ENV_URL + ' 写错、表结构缺列。两种后端都要先执行 sql/00-schema.sql 建表；'
      + '云端接着执行 sql/vector-search.sql 补 RLS 与 RPC，'
      + '自托管接着执行 docker/init/10-roles.sql 授权。',
  });
}
  ...
  if (r.status === 404) {
    ...
    if (!settingsMissingWarned) {
      ...
      process.stderr.write('[moyi] 未找到 settings 表：管理面板与全局记忆不可用，其余功能不受影响。'
        + '请执行 sql/00-schema.sql 补建（Supabase 之后还要执行 sql/vector-search.sql）。\n');
    }
...
async function authenticate(req) {
  ...
  if (!key) return null;
  // 浏览器授权签发的 access token（moat_ 前缀）：查 oauth_tokens 映射到其专属 Agent。
  // 未建表 / 未命中 / 过期 → null，与普通无效 key 同样走 401。
  if (key.startsWith('moat_')) return await OAUTH.authenticateAccessToken(supa, key);
...
const DB_MIN_SIM = numEnv('MOYI_DB_MIN_SIM', 0.05, { min: 0, max: 1 });
...
async function hybridSearch(aid, query, limit, settings) {
  ...
  // 权重不设上界也不归一：写坏了只影响相对排序，numEnv 至少保证它不是 NaN
  const W_VEC = numEnv('MOYI_W_VEC', 0.7);
  const W_KW = numEnv('MOYI_W_KW', 0.3);
...
/** 同意页表单用 x-www-form-urlencoded；上限压到 32KB（浏览器授权不需要大 body）。 */
function parseForm(req) {
  return new Promise(res => {
    ...
    req.on('data', c => {
      size += c.length;
      if (size > 32 * 1024) { req.destroy(); return res({}); }
      b += c;
    });
    req.on('end', () => {
      const out = {};
      try { new URLSearchParams(b).forEach((v, k) => { out[k] = v; }); } catch {}
      res(out);
    });
  });
}
function html(res, code, body, extraHeaders) {
  const h = Object.assign({ 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' },
    extraHeaders || {});
 ...
 *
 * CSRF：会话 Cookie 是 SameSite=Lax（跨站 POST 不带 Cookie），且所有写操作额外要求
...
async function handleConsole(req, res, p, method, u) {
  ...
  if (cp === '/setup' && method === 'POST') {
    ...
    if (pre.status === 404) {
      return void json(res, 409, { error: '数据库尚未初始化', hint: '先执行 sql/00-schema.sql；Supabase 之后还要执行 sql/vector-search.sql 补 RLS 与 RPC' });
    }
    ...
    if (st0.status === 404) {
      return void json(res, 409, { error: '缺少 settings 表，无法完成安装', hint: '先执行 sql/00-schema.sql；Supabase 之后还要执行 sql/vector-search.sql 补 RLS 与 RPC，再重试' });
    }
  ...
  // ── 技能管理（公共只读层的唯一写入口）────────────────
  // 权限取「任一已登录管理员」，与 Agent 发号同级：审核发布是运维动作，不是超管
  // 专属的策略动作。真正要挡的是 Agent 侧自助发布，那已由 handleSkills 里
  // status 服务端赋值挡住。
  if (cp === '/skills' || cp.startsWith('/skills/')) {
    return void await handleConsoleSkills(req, res, cp.slice('/skills'.length) || '/', method, sess);
  }
...
/**
 * 管理台侧的技能端点。
 *   GET    /api/console/skills            全量（含 draft/rejected）+ 计数
 *   POST   /api/console/skills/preview    抓一个 URL，回填推断出的元数据，不落库
 *   POST   /api/console/skills            新建（正文直填，或带 source_url 现抓）
 *   POST   /api/console/skills/:id/publish|reject   审核
 *   PATCH  /api/console/skills/:id        改正文/描述（改正文即重算 sha256）
 *   POST   /api/console/skills/:id/refresh 按 source_url 重抓，变了才写
 *   DELETE /api/console/skills/:id
 * 说明：Agent 提交的草稿在这里能被看到、被发布，也能被直接删。
 */
async function handleConsoleSkills(req, res, sp, method, sess) {
  if (sp === '/' && method === 'GET') {
    const all = await SK.listSkills(supa, 'id=not.is.null');
    if (all.missing) return json(res, 409, { error: '缺少 skills 表',
      hint: '请执行 sql/00-schema.sql（Supabase 还要执行 sql/vector-search.sql）' });
    if (all.failure) throw new DbError(all.failure);
    const count = st => all.rows.filter(r => r.status === st).length;
    return json(res, 200, {
      skills: all.rows,
      counts: { published: count('published'), draft: count('draft'), rejected: count('rejected') },
      fetch_hosts: SK.hostWhitelist(),
    });
  }

  // 预览：抓取 + 推断，不落库。管理员在表单里核对后自己决定要不要发布。
  if (sp === '/preview' && method === 'POST') {
    const b = await parseBody(req);
    const got = await SK.fetchSkillText(b.url);
    if (got.error) return json(res, 400, { error: got.error, hint: '可用主机白名单：' + SK.hostWhitelist().join(', ') });
    const meta = SK.inferMeta(got.content, got.sourceUrl);
    return json(res, 200, {
      name: meta.name, description: meta.description, source_url: got.sourceUrl,
      chars: got.content.length, bytes: Buffer.byteLength(got.content),
      content_head: got.content.slice(0, 600),
      note: '名称与描述由正文 frontmatter / 首个标题推出，可直接改',
    });
  }
  ...
  if (sp === '/' && method === 'POST') {
    ...
    let sourceUrl = b.source_url ? String(b.source_url).trim() : null;
    // 带 url 时以抓到的正文为准，忽略同时传来的 content —— 两者都传却取 content
    // 会造成「存下来的和链接指向的不是一回事」，这类不一致后期极难排查。
    if (sourceUrl && !content.trim()) {
      const got = await SK.fetchSkillText(sourceUrl);
      if (got.error) return json(res, 400, { error: got.error, hint: '可用主机白名单：' + SK.hostWhitelist().join(', ') });
      content = got.content;
      sourceUrl = got.sourceUrl;
    }
    const cerr = SK.checkContent(content);
    if (cerr) return json(res, 400, { error: cerr });
    const inferred = SK.inferMeta(content, sourceUrl);
    const name = String(b.name || inferred.name || '').trim();
    const nerr = SK.validName(name);
    if (nerr) return json(res, 400, { error: nerr });
    const rec = {
      name, description: String(b.description || inferred.description || '').slice(0, SK.MAX_DESC),
      content, sha256: SK.sha256(content),
      status: 'published',           // 人放的直接发布；Agent 提交走另一条路且被强制 draft
      origin: sourceUrl ? 'url' : 'console',
      source_url: sourceUrl, fetched_at: sourceUrl ? new Date().toISOString() : null,
      agent_id: null,
    };
    const out = await SK.insertSkill(supa, rec);
    if (out.conflict) return json(res, 409, { error: out.conflict });
    if (out.error) return json(res, 502, { error: out.error, detail: out.detail });
    audit(sess.id, 'console.skill_created', { name, origin: rec.origin });
    return json(res, 201, { skill: out.row });
  }

  const m = sp.match(/^\/([\w-]+)(?:\/(publish|reject|refresh))?$/);
  if (!m) return json(res, 404, { error: '路径不存在: /console/skills' + sp });
  const id = safeUuid(m[1]);
  if (!id) return json(res, 400, { error: 'id 格式不合法' });
  const action = m[2];

  // GET /api/console/skills/:id —— 管理台读单篇正文（管理员没有 Agent key，
  // 走不了 Agent 面的读取端点，这里给一条同等能力的管理员通道）。
  if (!action && method === 'GET') {
    const cur = await SK.getSkillById(supa, id);
    if (cur.missing) return json(res, 409, { error: '缺少 skills 表' });
    if (cur.failure) throw new DbError(cur.failure);
    if (!cur.row) return json(res, 404, { error: '技能不存在' });
    return json(res, 200, { skill: cur.row });
  }
  ...
  if (action === 'refresh' && method === 'POST') {
    ...
    if (cur.failure) throw new DbError(cur.failure);
    if (!cur.row) return json(res, 404, { error: '技能不存在' });
    if (!cur.row.source_url) return json(res, 400, { error: '这条技能没有来源地址，无从刷新' });
    const got = await SK.fetchSkillText(cur.row.source_url);
    if (got.error) return json(res, 400, { error: got.error });
    const nextSha = SK.sha256(got.content);
    if (nextSha === cur.row.sha256) {
      return json(res, 200, { changed: false, note: '内容未变（sha256 一致），未写库' });
    }
    const upd = await SK.updateSkill(supa, id, {
      content: got.content, sha256: nextSha, fetched_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    });
    if (upd.error) return json(res, 502, { error: upd.error });
    audit(sess.id, 'console.skill_refreshed', { target: id });
    return json(res, 200, { changed: true, skill: upd.row });
  }

  if (action && method === 'POST') {
    const next = action === 'publish' ? 'published' : 'rejected';
    const upd = await SK.updateSkill(supa, id, { status: next, updated_at: new Date().toISOString() });
    if (upd.error) return json(res, 404, { error: upd.error });
    audit(sess.id, 'console.skill_' + next, { target: id, name: upd.row.name });
    return json(res, 200, { skill: upd.row });
  }

  if (method === 'PATCH') {
    const b = await parseBody(req);
    const patch = { updated_at: new Date().toISOString() };
    if (b.description != null) patch.description = String(b.description).slice(0, SK.MAX_DESC);
    if (b.content != null) {
      const cerr = SK.checkContent(b.content);
      if (cerr) return json(res, 400, { error: cerr });
      patch.content = String(b.content);
      patch.sha256 = SK.sha256(patch.content);
    }
    if (Object.keys(patch).length <= 1) return json(res, 400, { error: '没有要改的字段' });
    const upd = await SK.updateSkill(supa, id, patch);
    if (upd.error) return json(res, 404, { error: upd.error });
    audit(sess.id, 'console.skill_updated', { target: id });
    return json(res, 200, { skill: upd.row });
  }

  if (method === 'DELETE') {
    const del = await SK.deleteSkill(supa, id);
    if (del.error) return json(res, 400, { error: del.error });
    if (!del.deleted) return json(res, 404, { error: '技能不存在' });
    audit(sess.id, 'console.skill_deleted', { target: id });
...
function mcpSnippet(key, req) {
  // 两种并列接入形态，任选其一贴进 MCP 客户端配置：
  //   1) stdio：本地跑 mcp-server.js，适合桌面客户端（Claude Desktop 等）。
  //   2) HTTP：直接连部署地址的 /api/mcp，适合能配远程 MCP 的客户端。
  const base = req ? publicOrigin(req) : 'http://localhost:3906';
  return JSON.stringify({
    mcpServers: { moyi: { command: 'node', args: ['/absolute/path/to/moyi/mcp-server.js'],
      env: { MOYI_API: 'http://localhost:3906', MOYI_KEY: key } } },
    // 远程 HTTP 形态：把上面这段换成下面这个即可，url 用你的部署地址
    _http_alternative: {
      mcpServers: { moyi: { url: base + '/api/mcp',
        headers: { Authorization: 'Bearer ' + key } } },
    },
  }, null, 2);
}

// 推断「用户实际访问的站点 origin」，用于生成远程 MCP 的 url。
// 优先反向代理传来的 X-Forwarded-Proto + Host；退化到 req.headers.host；都没有回本地。
function publicOrigin(req) {
  const host = (req.headers['x-forwarded-host'] || req.headers.host || '').split(',')[0].trim();
  if (!host) return 'http://localhost:3906';
  const proto = (req.headers['x-forwarded-proto'] || 'http').split(',')[0].trim();
  return proto.replace(/:\/\//g, '') + '://' + host;
}
...
/**
 * MCP HTTP 端点用：在「已鉴权的父请求」内，用同一份凭据再自调用一次 route()，
 * 打到某个既有 REST 端点，把结果收成对象返回。这样工具执行完全复用 REST 逻辑，
 * 不在 MCP 层复制一份业务代码。凭据从父请求头透传（x-moyi-key / authorization），
 * 不重新拼 key。
 * @returns {Promise<object>} REST 响应的 JSON（失败时为 {error:...}）
 */
function internalRoute(parentReq, method, path, body) {
  const u = new URL(path, 'http://mcp.internal');
  const req = new EventEmitter();
  req.method = method;
  req.url = u.pathname + (u.search || '');
  req.__mcpInternal = true;  // 防止内部自调用再命中 /mcp 造成递归
  const h = {};
  if (parentReq.headers['x-moyi-key']) h['x-moyi-key'] = parentReq.headers['x-moyi-key'];
  if (parentReq.headers['authorization']) h['authorization'] = parentReq.headers['authorization'];
  h['content-type'] = 'application/json';
  req.headers = h;
  req.socket = parentReq.socket || { remoteAddress: 'mcp' };
  req.destroy = () => { req.destroyed = true; };

  const buf = body != null ? Buffer.from(JSON.stringify(body)) : Buffer.alloc(0);
  req.on('newListener', (name) => {
    if (name !== 'data' && name !== 'end') return;
    queueMicrotask(() => {
      if (req._fed) return; req._fed = true;
      if (buf.length && !req.destroyed) req.emit('data', buf);
      req.emit('end');
    });
  });
  ...
  return new Promise(resolve => {
    let code = 200, raw = '';
    const res = {
      writeHead(c) { code = c; return res; },
      end(b) { if (b != null) raw += b; try { resolve({ _status: code, ...JSON.parse(raw || '{}') }); }
               catch { resolve({ _status: code, error: '内部路由未返回 JSON', raw }); } },
    };
    Promise.resolve(routeInner(req, res, null, u)).catch(e =>
      resolve({ _status: 500, error: e && e.message ? e.message : String(e) }));
  });
}

/**
 * MCP over HTTP 端点处理器（Streamable HTTP 的「JSON-RPC over POST」子集）。
 *   - POST /api/mcp  body 为单条 JSON-RPC 或数组批量 → 逐条派发，回 JSON。
 *   - GET  /api/mcp  本实现不启用 SSE 流，按规范回 405（客户端应据此只用 POST）。
 *   - OPTIONS        走通用 CORS 预检。
 * 通知类消息（无 id）不产生响应体；全为通知时回 202 空。
 */
async function handleMcp(req, res, method, agent) {
  if (method === 'GET') {
    return json(res, 405, { error: '本 MCP 端点仅支持 POST（JSON-RPC over HTTP）',
      hint: 'initialize / tools/list / tools/call 均以 POST 发送' });
  }
  if (method !== 'POST') return json(res, 405, { error: '方法不允许' });

  const b = await parseBody(req);
  const isBatch = Array.isArray(b);
  const msgs = isBatch ? b : [b];
  if (!msgs.length || msgs.some(m => !m || typeof m !== 'object' || !m.method)) {
    return json(res, 400, { error: '请求体需为 JSON-RPC 对象或其数组' });
  }

  const ctx = {
    agent,
    callRest: (m, path, body) => internalRoute(req, m, path, body),
  };
  const replies = [];
  for (const m of msgs) {
    const r = await MCP.handleRpc(m, ctx);
    if (r) replies.push(r);
  }

  if (!replies.length) {  // 全是通知：202 Accepted，无 body
    res.writeHead(202, corsHeaders(req));
    return res.end();
  }
  return json(res, 200, isBatch ? replies : replies[0]);
}

/**
 * MCP 浏览器授权端点集（挂在 Agent 面的 /api/oauth/* 下，早于 Key 闸门）：
 *   GET  /api/oauth/authorize   参数校验 + 出同意页（需管理员会话）
 *   POST /api/oauth/approve     同意页表单提交（同意→出码 302 / 拒绝→error 302）
 *   POST /api/oauth/token       授权码(+PKCE) / refresh 换 access+refresh
 * 未配置 MOYI_OAUTH_CLIENT_ID 时全部 501，等于整块功能关闭。
 */
async function handleOAuth(req, res, sub, method, u) {
  const origin = publicOrigin(req);
  const q = {};
  u.searchParams.forEach((v, k) => { q[k] = v; });

  if (!OAUTH.oauthEnabled()) {
    return json(res, 501, { error: 'unsupported_grant_type',
      error_description: '本实例未开启浏览器授权（未设置 MOYI_OAUTH_CLIENT_ID）' });
  }

  // GET /api/oauth/authorize —— 出同意页
  if (sub === 'authorize' && method === 'GET') {
    const v = OAUTH.validateAuthorizeRequest(q);
    if (!v.ok) {
      if (v.redirect) { res.writeHead(302, { Location: v.redirect, 'Cache-Control': 'no-store' }); return res.end(); }
      return html(res, 400, OAUTH.renderMessage((await getSettings()).instance_name,
        '授权请求无效', '该授权请求不合法：' + esc(v.desc) + '。请让客户端重新发起。'));
    }
    // 同意动作必须绑一个已登录管理员；未登录给一页「先去管理台登录」。
    const sess = await adminSession(req);
    if (!sess) return html(res, 200, OAUTH.renderLoginPrompt((await getSettings()).instance_name));
    return html(res, 200, OAUTH.renderConsentPage(q, (await getSettings()).instance_name));
  }

  // POST /api/oauth/approve —— 表单提交
  if (sub === 'approve' && method === 'POST') {
    // CSRF 主防线是 SameSite=Lax（跨站 POST 不带会话 Cookie），这里再补一道：
    // 浏览器对跨源表单 POST 一定会带 Origin；带了就必须等于本站。
    const ro = String(req.headers.origin || '').replace(/\/+$/, '');
    if (ro && ro !== origin.replace(/\/+$/, '')) return json(res, 403, { error: 'origin 校验失败' });
    const sess = await adminSession(req);
    if (!sess) return html(res, 401, OAUTH.renderLoginPrompt((await getSettings()).instance_name));
    const f = await parseForm(req);
    const v = OAUTH.validateAuthorizeRequest(f);
    if (!v.ok) return html(res, 400, OAUTH.renderMessage((await getSettings()).instance_name,
      '授权请求无效', '提交内容未通过与发起时一致的校验：' + esc(v.desc) + '。'));
    // 拒绝：带标准 error=access_denied 回跳（redirect 已在 validate 里确认白名单）
    if (f.decision !== 'approve') {
      const back = OAUTH.withErr(v.q, 'access_denied') || v.q.redirect_uri;
      res.writeHead(302, { Location: back, 'Cache-Control': 'no-store' });
      return res.end();
    }
    try {
      const out = await OAUTH.mintAuthCode(supa, {
        q: v.q,
        createAgent: async (label) => {
          const key = genKey();
          const r = await supa('/agents', 'POST', { name: label, api_key_hash: hashKey(key), role: 'agent' }, 'return=representation');
          if (r.status >= 400 || !(r.data || []).length) throw new Error('无法创建授权 Agent');
          return { agentId: r.data[0].id };
        },
      });
      if (out.error) return html(res, 500, OAUTH.renderMessage((await getSettings()).instance_name, '出错了', esc(out.error)));
      audit(sess.id, 'oauth.authorize', { client_id: OAUTH.oauthConfig().clientId, agent_id: out.agentId });
      res.writeHead(302, { Location: out.location, 'Cache-Control': 'no-store' });
      return res.end();
    } catch (e) {
      return html(res, 500, OAUTH.renderMessage((await getSettings()).instance_name, '出错了', esc(e.message || String(e))));
    }
  }

  // POST /api/oauth/token —— 换 token
  if (sub === 'token' && method === 'POST') {
    // 流只能消费一次：按 Content-Type 二选一解析。规范允许 token 端点
    // 接受 form-urlencoded，而浏览器授权客户端两家实现都有，都得接住。
    const ctype = String(req.headers['content-type'] || '');
    const body = ctype.includes('application/x-www-form-urlencoded')
      ? await parseForm(req) : await parseBody(req);
    try {
      const out = await OAUTH.handleTokenBody(supa, body);
      return json(res, out.status, out.json, out.headers);
    } catch (e) {
      return json(res, 500, { error: 'server_error', error_description: e.message }, { 'Cache-Control': 'no-store' });
    }
  }

  return json(res, 404, { error: '未知的 OAuth 端点: /oauth/' + sub });
}
...
/**
 * Agent 面的技能端点。三条不变量：
 *   1. Agent 只能读到 published，外加「自己提交且仍是 draft」的那几份。
 *      别人的草稿按 404 处理（不是 403）——草稿是否存在本身就是不该泄露的信息。
 *   2. status / origin / agent_id 全部服务端赋值，客户端传值一概忽略。
 *   3. Agent 提交永远落在 draft，进不了公共层；发布是人的动作。
 */
async function handleSkills(req, res, sub, method, agent, u) {
  const aid = safeUuid(agent.id);

  // GET /api/skills —— 公共清单 + 自己的草稿
  if (sub === '/' && method === 'GET') {
    const pub = await SK.listSkills(supa, 'status=eq.published');
    if (pub.missing) return json(res, 409, { error: '缺少 skills 表',
      hint: '请执行 sql/00-schema.sql（Supabase 还要执行 sql/vector-search.sql）' });
    if (pub.failure) throw new DbError(pub.failure);
    const mine = aid ? await SK.listSkills(supa, 'status=eq.draft&agent_id=eq.' + aid) : { rows: [] };
    if (mine.failure) throw new DbError(mine.failure);
    return json(res, 200, {
      skills: pub.rows, drafts: mine.rows || [],
      note: 'draft 只有你自己可见，需管理员在管理台发布后才会对其它 Agent 开放',
    });
  }

  // POST /api/skills —— Agent 提交一份草稿
  if (sub === '/' && method === 'POST') {
    const b = await parseBody(req);
    const content = String(b.content || '');
    const cerr = SK.checkContent(content);
    if (cerr) return json(res, 400, { error: cerr });
    const inferred = SK.inferMeta(content, b.url);
    const name = String(b.name || inferred.name || '').trim();
    const nerr = SK.validName(name);
    if (nerr) return json(res, 400, { error: nerr, hint: '也可不传 name，由正文的 frontmatter 或首个标题推出' });
    const rec = {
      name,
      description: String(b.description || inferred.description || '').slice(0, SK.MAX_DESC),
      content, sha256: SK.sha256(content),
      status: 'draft', origin: 'agent', agent_id: aid,
    };
    const out = await SK.insertSkill(supa, rec);
    if (out.conflict) return json(res, 409, { error: out.conflict, hint: '换一个 name，或让管理员更新已有的那条' });
    if (out.error) return json(res, 502, { error: out.error, detail: out.detail });
    audit(aid, 'skill.draft_created', { name, source_url: b.url || null });
    return json(res, 201, { skill: publicSkill(out.row),
      status: 'draft', note: '已存为草稿，等待管理员在管理台发布后才对所有 Agent 可见' });
  }

  // GET /api/skills/:name —— 读单篇
  const nm = sub.match(/^\/([\w.-]+)$/);
  if (nm && method === 'GET') {
    const r = await SK.getSkillByName(supa, decodeURIComponent(nm[1]));
    if (r.missing) return json(res, 409, { error: '缺少 skills 表',
      hint: '请执行 sql/00-schema.sql（Supabase 还要执行 sql/vector-search.sql）' });
    if (r.failure) throw new DbError(r.failure);
    const row = r.row;
    const visible = row && (row.status === 'published'
      || (row.status === 'draft' && aid && String(row.agent_id) === aid));
    if (!visible) return json(res, 404, { error: '技能不存在：' + nm[1] });
    return json(res, 200, { skill: publicSkill(row), content: row.content });
  }

  return json(res, 404, { error: '未知的技能端点: /skills' + sub });
}

/** 对外形状：绝不把内部字段（如 sha256 之外的存储细节）漏出去。 */
function publicSkill(row) {
  if (!row) return null;
  return {
    name: row.name, description: row.description, status: row.status,
    origin: row.origin, source_url: row.source_url || null,
    uri: SK.resourceUri(row.name), updated_at: row.updated_at,
  };
}
...
async function routeInner(req, res, pathname, url) {
  ...
  // ── MCP 浏览器授权（OAuth 2.1 精简子集）──
  // 这是「认证面本身」：自带会话校验与 Origin 校验，绝不能落到下面的
  // Agent Key 闸门，否则拿不到 code 也换不了 token（鸡生蛋问题）。
  if (p === '/oauth' || p.startsWith('/oauth/')) {
    return await handleOAuth(req, res, p.slice('/oauth/'.length), method, u);
  }
  // RFC 8414 / RFC 9728 发现文档。路径在站点根（不在 /api 下），由
  // server.js / webentry.js 特意放行到这里；routeInner 只 strip /api 前缀，
  // 所以 p 保持 /.well-known/... 原样。
  if (method === 'GET' && (p === '/.well-known/oauth-authorization-server'
    ...
    const origin = publicOrigin(req);
    const meta = p.includes('authorization-server')
      ? OAUTH.asMetadata(origin) : OAUTH.protectedResourceMetadata(origin);
    return json(res, 200, meta, { 'Cache-Control': 'public, max-age=300' });
  }
  ...
  if (!agent) {
    ...
    // 受保护资源元数据发现（RFC 9728）：MCP 客户端拿到 401 后据此去读
    // /.well-known/oauth-protected-resource，进而走浏览器授权。仅 /api/mcp
    // 且确实开启了 OAuth 时才带这个头，避免误导未开启 OAuth 的部署。
    const hint = {};
    if (p === '/mcp' && OAUTH.oauthEnabled()) {
      hint['WWW-Authenticate'] = 'Bearer resource_metadata="'
        + publicOrigin(req) + '/.well-known/oauth-protected-resource"';
    }
    return json(res, 401, { error: '需要有效的 API Key（X-Moyi-Key 头）',
      hint: p === '/mcp' && OAUTH.oauthEnabled()
        ? '可用浏览器授权获取 access token，见 /.well-known/oauth-authorization-server' : undefined }, hint);
  }
  clearAuthFail(req);
  const aid = agent.id;
  ...
  // ── MCP over HTTP（Streamable HTTP：JSON-RPC 走 POST）──
  // 认证已在上游完成（Agent 面 key / Bearer），这里只做协议派发。
  // 工具执行通过 internalRoute 复用既有 REST 端点，逻辑不在此重复。
  if (p === '/mcp') {
    if (req.__mcpInternal) return json(res, 400, { error: 'MCP 端点不接受内部自调用' });
    return await handleMcp(req, res, method, agent);
  }

  // ── 技能（Agent 面：读公共已发布层 + 自己那份草稿）──
  // status 与 origin 一律由服务端决定，绝不接受客户端传值：否则一个 Agent
  // 只要在提交时多写一个 "status":"published" 就绕过了整道人工审核闸。
  if (p === '/skills' || p.startsWith('/skills/')) {
    return await handleSkills(req, res, p.slice('/skills'.length) || '/', method, agent, u);
  }
  ...
  if (p === '/decay/preview' && method === 'GET') {
    ...
    return json(res, 200, {
      ...
      forget_after_days: FORGET_AFTER,
      scanned: rows.length,
      pinned: ev.pinned,
      keep: ev.keep.length,
