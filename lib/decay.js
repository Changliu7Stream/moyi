...
const HALFLIFE_DAYS = Math.max(7, Number(process.env.MOYI_DECAY_HALFLIFE || 90));
/** 检索排序时的活力地板 */
const FLOOR = { high: 0.55, medium: 0.30, low: 0.0 };
/** 低于此「原始衰减」才降级（high 不参与自动降级，见上） */
const DOWNGRADE_BELOW = { medium: 0.15, low: 0 };
const FORGET_AFTER = Math.max(30, Number(process.env.MOYI_FORGET_AFTER_DAYS || 180));
const FORGET_MAX_ACCESS = Number(process.env.MOYI_FORGET_MAX_ACCESS || 1);
